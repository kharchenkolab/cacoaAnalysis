...
sapply(cao_ep$test.results$gp.local$sim.scores, function(sc) setNames(p_genes %in% names(sc), local_prog_genes))
```
...
plot_grid(
  ...
  plot_grid(plotlist=rast(c(
    ...
    labels=c(
      "", "SHISA9, Control", "SHISA9, MS", "", "",
      "", glob_genes,
      "", names(ggs_local_genes)),
    label_fontface="italic", hjust=0, label_x=0.06, label_size=11,
    nrow=3, ncol=5, rel_widths=c(2, 1, 1, 1, 1), rel_heights=c(2, 2, 1), scale=scale
  ),
...
```{r, fig.width=8.5, fig.height=5.6, message=FALSE, warning=FALSE}
scale <- 0.95
