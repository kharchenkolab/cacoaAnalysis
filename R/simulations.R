...
generateDistanceMatrixSimulations <- function(mean.offset=0, std.cnt=1, std.disease=1, ...) {
  lapply(c("within", "within.balanced", "within.ref"), function(n.type) {
    lapply(c(5, 10, 20, 50), function(n.cnt) {
      lapply(c(5, 10, 20, 50), function(n.disease) {
        
        sapply(1:50, function(rep) {
          generateDistanceMatrix(n.cnt=n.cnt, n.disease=n.disease, mean.offset=mean.offset, 
                                 std.cnt=std.cnt, std.disease=std.disease) %$%
            median(normalizedExpressionShiftsForOne(p.dists, sample.groups, norm.type=n.type, ...))
        }) %>% {data.frame(mean.offset=mean.offset, std.cnt=std.cnt, std.disease=std.disease,
                           dist=., norm.type=n.type, n.cnt=n.cnt, n.disease=n.disease)}
